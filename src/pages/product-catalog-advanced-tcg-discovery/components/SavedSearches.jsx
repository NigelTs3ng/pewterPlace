...
const SavedSearches = ({ onApplySearch }) => {
  const [savedSearches, setSavedSearches] = useState([
    ...
    },
    {
      ...
      alertEnabled: false,
      lastUpdated: '2025-08-29T15:45:00Z',
      resultCount: 234
    },
    {
      id: 3,
  ...
  const getFilterSummary = (filters) => {
    ...
    if (filters?.conditions?.length) {
      parts?.push(`${filters?.conditions?.length} conditions`);
    }
    if (filters?.rarities?.length) {
      parts?.push(`${filters?.rarities?.length} rarities`);
    }
    if (filters?.priceMax) {
      parts?.push(`under $${filters?.priceMax}`);
    }
    if (filters?.gradedOnly) {
      parts?.push('graded only');
    }
